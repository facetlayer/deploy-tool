...
|---|---|---|
| `project-name=<name>` | both | The project. Must already be registered on the destination with `deploy create-project` (see [README](../README.md)). Note the name is *not* what authorizes the deploy — the instance's resource binding is. |
...
resolves every `include` against `deploy/` and ships an empty bundle. The
CLI's `--local-dir` flag, where present, wins over the config value.
Resolution is lexical — the path need not exist yet, so a `before-deploy` step
may create it.
